...
for step in np.arange(0, 100, 1):
    scaler = 2
    healthy = 100 - step
    numSickPos = round(V*step)
    numSickNeg = step - numSickPos
    ...
    fig.add_trace(go.Scatter(
        ...
        y=[1.5, 1, 1.5],
        mode='markers',
        marker=dict(size=[scaler*(numSickPos+numHealthyPos), scaler*100, scaler*(numSickNeg+numHealthyNeg)],
    ...
    fig.add_trace(go.Scatter(
        ...
        y=[1.5, 1, 1.5],
        mode='markers',
        marker=dict(size=[scaler*(numSickPos), scaler*step, scaler*(numSickNeg)],
                    color=['red', 'red', 'red'], opacity = 1),
        name="Sick"
    ))
